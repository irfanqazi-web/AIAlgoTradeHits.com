...
import { TrendingUp, TrendingDown, Activity, Check, ArrowUpDown, Sun, Moon, Search, Mic, MicOff, X, ChevronDown, ChevronUp, Layers } from 'lucide-react';
import { createChart } from 'lightweight-charts';
import { API_CONFIG } from '@/lib/config';
import { marketDataService } from '@/lib/services';
import ProfessionalChart from '../charts/ProfessionalChart';
import NestedSignals from '../signals/NestedSignals';
...
export default function TradingDashboard({ searchQuery, searchMethod }) {
  ...
  const [showNestedSignals, setShowNestedSignals] = useState(true); // Nested ML signals panel
  ...
  return (
    <div style={{ padding: '24px', background: currentTheme.background, minHeight: '100vh' }}>
      ...
      {/* Nested Multi-Timeframe ML Signals */}
      {activeMarket === 'stock' && (
        <div style={{ marginBottom: '24px' }}>
          {/* Toggle Header */}
          <button
            onClick={() => setShowNestedSignals(!showNestedSignals)}
            style={{
              width: '100%',
              padding: '16px 24px',
              background: showNestedSignals
                ? 'linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%)'
                : currentTheme.cardBg,
              color: showNestedSignals ? '#ffffff' : currentTheme.text,
              border: `1px solid ${showNestedSignals ? 'transparent' : currentTheme.cardBorder}`,
              borderRadius: showNestedSignals ? '16px 16px 0 0' : '16px',
              fontSize: '16px',
              fontWeight: '600',
              cursor: 'pointer',
              display: 'flex',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <Layers size={20} />
              <span>Nested ML Signals (Daily &gt; Hourly &gt; 5-Min)</span>
              <span style={{
                padding: '4px 10px',
                borderRadius: '12px',
                fontSize: '11px',
                fontWeight: '600',
                background: showNestedSignals ? 'rgba(255,255,255,0.2)' : `${currentTheme.primary}22`,
                color: showNestedSignals ? '#ffffff' : currentTheme.primary
              }}>
                66.2% UP Accuracy
              </span>
            </div>
            {showNestedSignals ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
          </button>

          {/* Nested Signals Panel */}
          {showNestedSignals && (
            <div style={{
              ...
              <NestedSignals
                onSymbolSelect={(symbol) => {
                  // Find the item in current data or create a minimal object
                  const existingItem = data.find(item => item.symbol === symbol);
                  if (existingItem) {
                    setSelectedItem(existingItem);
                  } else {
                    setSelectedItem({ symbol, pair: symbol });
                  }
                }}
                theme={theme}
              />
