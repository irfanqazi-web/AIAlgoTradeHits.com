#!/usr/bin/env python3
"""
FAST Backfill Missing 10 Stocks - $229 TwelveData Plan
Uses 2-second delays (800 credits/min allowed on paid plan)
"""
...
# Only the 10 missing stocks
MISSING_STOCKS = ['AIG', 'CL', 'CMCSA', 'LMT', 'MDT', 'PYPL', 'USB', 'WMT', 'XOM', 'ZTS']

# Stock metadata
STOCK_INFO = {
    'AIG': {'name': 'American International Group', 'sector': 'Financial Services', 'industry': 'Insurance', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
    'CL': {'name': 'Colgate-Palmolive Company', 'sector': 'Consumer Defensive', 'industry': 'Household Products', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
    'CMCSA': {'name': 'Comcast Corporation', 'sector': 'Communication Services', 'industry': 'Entertainment', 'exchange': 'NASDAQ', 'mic_code': 'XNAS'},
    'LMT': {'name': 'Lockheed Martin Corporation', 'sector': 'Industrials', 'industry': 'Aerospace & Defense', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
    'MDT': {'name': 'Medtronic plc', 'sector': 'Healthcare', 'industry': 'Medical Devices', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
    'PYPL': {'name': 'PayPal Holdings Inc.', 'sector': 'Financial Services', 'industry': 'Credit Services', 'exchange': 'NASDAQ', 'mic_code': 'XNAS'},
    'USB': {'name': 'U.S. Bancorp', 'sector': 'Financial Services', 'industry': 'Banks', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
    'WMT': {'name': 'Walmart Inc.', 'sector': 'Consumer Defensive', 'industry': 'Discount Stores', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
    'XOM': {'name': 'Exxon Mobil Corporation', 'sector': 'Energy', 'industry': 'Oil & Gas', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
    'ZTS': {'name': 'Zoetis Inc.', 'sector': 'Healthcare', 'industry': 'Drug Manufacturers', 'exchange': 'NYSE', 'mic_code': 'XNYS'},
}
...
def calculate_rsi(series, period=14):
    ...
    delta = series.diff()
    # Changed: Use Wilder's RMA (ewm with alpha=1/period) instead of SMA
...
def calculate_bollinger_bands(series, period=20, std_dev=2):
    """Bollinger Bands with ddof=0 (population std dev) - TradingView/Industry Standard"""
    sma = calculate_sma(series, period)
    # Changed: Use ddof=0 (population std dev) instead of ddof=1 (sample std dev)
...
def calculate_stochastic(high, low, close, k_period=14, d_period=3):
    ...
    # Changed: Fast to Slow Stochastic
    raw_k = 100 * ((close - lowest_low) / (highest_high - lowest_low))
    # %K is now smoothed (Slow Stochastic)
...
def calculate_adx(high, low, close, period=14):
    ...
    # Changed: Use Wilder's RMA (ewm with alpha=1/period) instead of regular EMA
...
def calculate_roc(series, period=10):
    """ROC with period=10 (TradingView default) - Changed from 12 to 10"""
...
def calculate_ichimoku(high, low, close, tenkan=9, kijun=26, senkou_b=52):
    ...
    senkou_b_raw = (high.rolling(window=senkou_b, min_periods=senkou_b).max() + low.rolling(window=senkou_b, min_periods=senkou_b).min()) / 2
    # Changed: Apply 26-period FORWARD shift to Senkou A and B (TradingView standard)
...
def main():
    print("=" * 60)
    print("FAST BACKFILL - 10 MISSING STOCKS")
    print("$229 TwelveData Plan - 2 second delays")
    print("=" * 60)
    print(f"Using API Key: {TWELVEDATA_API_KEY[:10]}...")
    print(f"Stocks to process: {MISSING_STOCKS}")
    print()
    ...
    for i, symbol in enumerate(MISSING_STOCKS, 1):
        print(f"\n{'='*40}")
        print(f"Stock {i}/{len(MISSING_STOCKS)}: {symbol}")
        print(f"{'='*40}")
        ...
        # $229 plan: 800 credits/min = 2 second delay is safe
        if i < len(MISSING_STOCKS):
            print(f"\n  Waiting 2 seconds (paid plan rate)...")
            time.sleep(2)
    ...
    print("=" * 60)
    print(f"Successful: {successful}/{len(MISSING_STOCKS)}")
