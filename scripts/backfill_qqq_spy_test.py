#!/usr/bin/env python3
"""
Backfill QQQ and SPY with ALL 97 Fields - Test Script
Fetches complete data, calculates all indicators, for Saleem to validate
"""
...
def calculate_all_indicators(df):
    ...
    print("    Calculating basic price metrics...")
    ...
    print("    Calculating momentum indicators (RSI, MACD, Stochastic)...")
    ...
    print("    Calculating moving averages (SMA, EMA, KAMA)...")
    ...
    print("    Calculating volume indicators (OBV, MFI, CMF)...")
    ...
    print("    Calculating returns and derived metrics...")
    ...
    print("    Calculating MFI, CMF, Ichimoku...")
    ...
    print("    Calculating VWAP and Volume Profile...")
...
def fetch_historical_data(symbol, outputsize=5000):
    ...
    print(f"  Fetching {outputsize} days of historical data for {symbol}...")
...
def fetch_fundamental_data(symbol):
    ...
    if symbol in etf_info:
        return etf_info[symbol]

    # Try API for other symbols
...
def update_bigquery_table(symbol, df, fundamental_data):
    """Update BigQuery table for a symbol"""
    print(f"  Updating BigQuery for {symbol}...")

    # Add metadata
    df['symbol'] = symbol
    df['name'] = fundamental_data.get('name', '')
    df['sector'] = fundamental_data.get('sector', '')
    df['industry'] = fundamental_data.get('industry', '')
    df['asset_type'] = fundamental_data.get('asset_type', 'ETF')
...
def save_to_csv(symbol, df):
    """Save data to CSV for validation"""
...
def main():
    print("=" * 70)
    print("QQQ AND SPY TEST BACKFILL - ALL 97 FIELDS")
    print("=" * 70)
    ...
    for symbol in TEST_SYMBOLS:
        ...
        # Fetch historical data (5000 days max to get full history)
        ...
        print(f"  Name: {fundamental_data.get('name', 'N/A')}")
        print(f"  Sector: {fundamental_data.get('sector', 'N/A')}")

        # Calculate all indicators
        print(f"\n  Calculating all 97 fields...")
        df = calculate_all_indicators(df)

        # Save to CSV for Saleem to validate
        csv_path = save_to_csv(symbol, df)

        # Update BigQuery
        rows = update_bigquery_table(symbol, df, fundamental_data)

        print(f"\n  SUCCESS: {rows} rows uploaded for {symbol}")
        print(f"  CSV file available at: {csv_path}")

        # Show sample of data
        print(f"\n  Sample of last 3 rows:")
        sample_cols = ['datetime', 'open', 'high', 'low', 'close', 'volume', 'rsi', 'macd', 'sma_200', 'adx']
    ...
    print("TEST COMPLETE")
    print("=" * 70)
    print(f"\nCSV files generated for Saleem to validate:")
    for symbol in TEST_SYMBOLS:
        print(f"  - C:/1AITrading/Trading/{symbol}_all_97_fields.csv")
    print("\nPlease review the CSV files to verify data quality before proceeding with all 106 stocks.")
