#!/usr/bin/env python3
"""
Consolidate Crypto Data into crypto_daily_clean table
Migrates data from cryptobot-462709 to aialgotradehits and calculates all 97 indicators
"""
...
SOURCE_TABLE = 'v2_daily_crypto'  # 196,231 rows

TARGET_PROJECT = 'aialgotradehits'
TARGET_DATASET = 'crypto_trading_data'
TARGET_TABLE = 'crypto_daily_clean'
...
# Top 50 cryptos to process (by market cap)
TOP_CRYPTOS = [
    ...
    'UNI/USD', 'NEAR/USD', 'XMR/USD', 'ETC/USD', 'FIL/USD',
    'APT/USD', 'ARB/USD', 'ALGO/USD', 'AAVE/USD', 'VET/USD',
    'XLM/USD', 'HBAR/USD', 'MKR/USD', 'OP/USD', 'GRT/USD',
    'INJ/USD', 'FTM/USD', 'SAND/USD', 'MANA/USD', 'MATIC/USD',
    'CRO/USD', 'IMX/USD', 'THETA/USD', 'RUNE/USD', 'LDO/USD',
    'CFX/USD', 'SNX/USD', 'RPL/USD', 'COMP/USD', 'ZEC/USD',
    'DASH/USD', 'EOS/USD', 'BAT/USD', 'FLOW/USD', 'XTZ/USD'
]
...
def fetch_crypto_data(symbol, outputsize=5000):
    """Fetch crypto data from TwelveData API"""
...
def calculate_all_indicators(df, symbol):
    ...
    # Metadata
    crypto_name = symbol.replace('/USD', '').replace('/', '')
    df['name'] = crypto_name
    df['sector'] = 'Cryptocurrency'
    df['industry'] = 'Digital Assets'
...
def delete_existing_data(symbol):
    ...
    query = f"""
    DELETE FROM `{TARGET_PROJECT}.{TARGET_DATASET}.{TARGET_TABLE}`
    WHERE symbol = '{symbol}'
    """
    try:
...
def upload_to_bigquery(df, symbol):
    """Upload to BigQuery"""
    table_id = f"{TARGET_PROJECT}.{TARGET_DATASET}.{TARGET_TABLE}"
    ...
    print(f"  Uploaded {len(df)} rows for {symbol}")

def process_crypto(symbol):
    """Process a single crypto"""
    print(f"\nProcessing {symbol}...")

    df = fetch_crypto_data(symbol, outputsize=5000)
...
def main():
    print("=" * 60)
    print("CONSOLIDATE CRYPTO INTO DAILY CLEAN")
    print("=" * 60)
    print(f"Target: {TARGET_PROJECT}.{TARGET_DATASET}.{TARGET_TABLE}")
    print(f"Processing {len(TOP_CRYPTOS)} cryptos")
    print()

    successful = 0
    failed = []

    for i, symbol in enumerate(TOP_CRYPTOS, 1):
        print(f"\n{'='*40}")
        print(f"Crypto {i}/{len(TOP_CRYPTOS)}: {symbol}")
        print(f"{'='*40}")

        try:
            success = process_crypto(symbol)
            if success:
                successful += 1
            else:
    ...
    print("=" * 60)
    print(f"Successful: {successful}/{len(TOP_CRYPTOS)}")
    if failed:
