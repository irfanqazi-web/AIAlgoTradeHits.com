"""
Create stocks_hourly_clean table with complete 85-field schema
Hourly data for 60 symbols, 4 months history
Partitioned by DAY (not MONTH - only 4 months = ~120 days)
"""
...
# Define the 85-field schema (identical to daily table)
schema = [
    ...
    bigquery.SchemaField("return_2w", "FLOAT", mode="NULLABLE",
                        description="Forward Return (2-week equivalent)"),
    bigquery.SchemaField("return_4w", "FLOAT", mode="NULLABLE",
                        description="Forward Return (4-week equivalent)"),
...
table_id = "stocks_hourly_clean"
full_table_id = f"aialgotradehits.{dataset_id}.{table_id}"

# Create table with partitioning and clustering
table = bigquery.Table(full_table_id, schema=schema)

# Partition by DAY (4 months = ~120 days, well within 4000 limit)
...
table.description = """
Clean consolidated hourly stock data table with 85 fields.
Includes OHLCV data, 40+ technical indicators, and ML features.
Source: v2_stocks_hourly
Covers 60 symbols with 4 months of hourly data.
Updated every hour during market hours (9 AM - 4 PM ET).
"""
...
try:
    ...
    print("\nNext steps:")
    print("1. Migrate hourly data from v2_stocks_hourly")
    print("2. Calculate all technical indicators")
    print("3. Set up Cloud Function for hourly updates")
