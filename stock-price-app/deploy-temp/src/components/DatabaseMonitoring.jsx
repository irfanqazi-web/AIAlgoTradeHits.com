...
function DatabaseMonitoring() {
  ...
  const fetchMonitoringData = async () => {
    ...
    const result = await monitoringService.getFullReport()
  ...
  const downloadPDF = () => {
    ...
    yPos += 6
    doc.text(`Total Records: ${monitoringService.formatNumber(summary.total_records || 0)}`, 14, yPos)
    yPos += 6
    ...
    const tableData = tables.map(table => [
      table.table_name,
      monitoringService.formatNumber(table.row_count || 0),
    ...
    if (topPairs.length > 0) {
      ...
      const pairsData = topPairs.map((pair, idx) => [
        ...
        monitoringService.formatNumber(pair.avg_volume_24h),
        pair.data_points
      ])
    ...
    return (
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginBottom: '30px' }}>
          ...
          <SummaryCard
            title="Total Records"
            value={monitoringService.formatNumber(summary.total_records || 0)}
            icon={Activity}
            color="#8b5cf6"
          />
          <SummaryCard
                ...
                {tables.map((table, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #334155' }}>
                    ...
                    <td style={{ padding: '12px', textAlign: 'right', color: '#10b981' }}>
                      {monitoringService.formatNumber(table.recent_24h_count || 0)}
                    </td>
              ...
              {monitoringData.quality.map((quality, idx) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '12px',
                    padding: '16px',
                    border: `1px solid ${monitoringService.getStatusColor(quality.status)}`
                  }}
                >
