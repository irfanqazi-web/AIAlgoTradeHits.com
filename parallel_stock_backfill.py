#!/usr/bin/env python3
"""
PARALLEL Index Stock Data Backfill Script
Optimized for TwelveData $229 Pro Plan (1,597 credits/minute)

Performance Target: 500+ symbols/hour (vs 107/hour sequential)

Key Optimizations:
1. ThreadPoolExecutor with 15 concurrent workers
2. Rate limiting: 750 API calls/minute (safe margin under 1,597)
3. Batch BigQuery uploads (50 symbols at a time)
4. Connection pooling with requests.Session
5. Resume capability from progress file
"""
...
RATE_LIMIT_PER_MINUTE = 750  # Safe margin under 1,597 credits
BATCH_SIZE = 50  # BigQuery batch upload size
...
def fetch_historical_data(symbol, outputsize=5000):
    """Fetch historical daily data from TwelveData API with rate limiting"""
...
def fetch_profile_data(symbol):
    """Fetch company profile from TwelveData with rate limiting"""
    rate_limiter.acquire()
    session = get_session()
...
def process_symbol(symbol):
    """Process a single symbol - returns (symbol, df, meta, profile) or (symbol, None, error)"""
    try:
        df, meta_or_error = fetch_historical_data(symbol, outputsize=5000)

        if df is None:
            return (symbol, None, None, meta_or_error)

        profile = fetch_profile_data(symbol)
        df = calculate_all_indicators(df)

        return (symbol, df, meta_or_error if isinstance(meta_or_error, dict) else {}, profile)
    except Exception as e:
        return (symbol, None, None, str(e))


def prepare_dataframe(symbol, df, meta_data, profile_data):
...
def batch_upload_to_bigquery(dataframes_dict):
    """Batch upload multiple symbols to BigQuery"""
    if not dataframes_dict:
        return 0

    # Delete existing data for all symbols in batch
    symbols = list(dataframes_dict.keys())
    symbols_str = "', '".join(symbols)
    delete_query = f"DELETE FROM `{PROJECT_ID}.{DATASET_ID}.{TABLE_NAME}` WHERE symbol IN ('{symbols_str}')"

    try:
        client.query(delete_query).result()
    except Exception as e:
        print(f"Warning: Delete query failed: {e}")

    # Combine all dataframes
    combined_df = pd.concat(list(dataframes_dict.values()), ignore_index=True)
...
def atomic_update_progress(progress, completed=None, failed=None, records=0):
...
def main():
    print("=" * 80)
    print("PARALLEL INDEX STOCK BACKFILL - OPTIMIZED FOR PERFORMANCE")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Total symbols: {len(ALL_SYMBOLS)}")
    print(f"Workers: {MAX_WORKERS} | Rate limit: {RATE_LIMIT_PER_MINUTE}/min | Batch size: {BATCH_SIZE}")
    print("=" * 80)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                symbol, df, meta, profile_or_error = result

                if df is not None:
                    prepared_df = prepare_dataframe(symbol, df, meta, profile_or_error)
                    batch_results[symbol] = prepared_df
                    processed_count += 1

                    date_range = f"{df['datetime'].min().date()} to {df['datetime'].max().date()}"
                    print(f"[{processed_count}/{len(ALL_SYMBOLS)}] {symbol}: OK ({len(df)} rows, {date_range})")

                    # Upload batch when full
                    if len(batch_results) >= BATCH_SIZE:
                        print(f"\n>>> Uploading batch of {len(batch_results)} symbols to BigQuery...")
                        rows = batch_upload_to_bigquery(batch_results)

                        for sym in batch_results.keys():
                            atomic_update_progress(progress, completed=sym, records=len(batch_results[sym]))

                        print(f">>> Batch uploaded: {rows:,} rows\n")
                        batch_results = {}
                else:
                    print(f"[{processed_count}/{len(ALL_SYMBOLS)}] {symbol}: FAILED - {profile_or_error}")
                    atomic_update_progress(progress, failed=symbol)
    ...
    if batch_results:
        print(f"\n>>> Uploading final batch of {len(batch_results)} symbols...")
        rows = batch_upload_to_bigquery(batch_results)
        for sym in batch_results.keys():
            atomic_update_progress(progress, completed=sym, records=len(batch_results[sym]))
        print(f">>> Final batch uploaded: {rows:,} rows")

    # Final report
    elapsed = (datetime.now() - datetime.fromisoformat(progress['start_time'])).total_seconds() / 3600
    print("\n" + "=" * 80)
    ...
    print(f"Rate: {len(progress['completed'])/elapsed:.1f} symbols/hour")
    print("=" * 80)
