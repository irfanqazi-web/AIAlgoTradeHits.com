...
export default function TableInventory({ theme = 'dark' }) {
  ...
  const downloadXLSX = async (tableName) => {
    ...
    try {
      console.log('📊 Starting Excel download for', tableName);

      // Fetch data if not already loaded (limit: 1000)
      let dataToExport = fullTableData;
      if (!dataToExport || dataToExport.length === 0) {
        try {
          console.log('Fetching table data (limit: 1000)...');
          const response = await fetch(`${API_BASE_URL}/api/admin/table-data/${tableName}?limit=1000`);
          const result = await response.json();
          if (result.success && result.data && result.data.length > 0) {
            dataToExport = result.data;
            setFullTableData(result.data);
            console.log(`✅ Fetched ${result.data.length} rows for ${tableName}`);
          } else {
      ...
      const structureData = schema.map((field, idx) => ({
        '#': idx + 1,
      ...
      // Set column widths for Structure tab
      wsStructure['!cols'] = [
        { wch: 5 },   // #
        { wch: 25 },  // Field Name
        { wch: 15 },  // Type
      ...
      // TAB 2: Data
      if (dataToExport && dataToExport.length > 0) {
        console.log(`Creating Data tab with ${dataToExport.length} rows...`);

        // Create worksheet from data
        const wsData = XLSX.utils.json_to_sheet(dataToExport);

        // Auto-size columns (set reasonable max width)
        const headers = Object.keys(dataToExport[0]);
        wsData['!cols'] = headers.map(h => ({ wch: Math.min(h.length + 5, 30) }));

        XLSX.utils.book_append_sheet(wb, wsData, 'Data');
        console.log('✅ Data tab created successfully');
      } else {
        // Create error/info sheet
        const errorData = [
          { Status: 'Error', Message: dataError || 'No data available for this table' },
          { Status: 'Table', Message: tableName },
          { Status: 'Suggestion', Message: 'Download limited to 1000 rows for analysis' }
        ];

        const wsError = XLSX.utils.json_to_sheet(errorData);
        wsError['!cols'] = [{ wch: 15 }, { wch: 80 }];

        XLSX.utils.book_append_sheet(wb, wsError, 'Data');
        console.log('⚠️ Created Data tab with error information');
      }
      ...
      if (dataError) {
        alert(`Excel file created with schema, but data fetch failed: ${dataError}\n\nThe Data tab shows error information. Try the CSV Stream download for large tables.`);
      } else {
        // Success message
        console.log(`✅ Successfully created ${filename} with ${dataToExport?.length || 0} rows`);
      }
    } catch (err) {
  ...
  const downloadCSVStream = (tableName) => {
    const limit = 1000; // Max 1000 rows
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <button
                  ...
                  title="Stream download - limited to 1000 rows for analysis"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '6px',
                ...
                <button
                  ...
                  title="Excel with Structure + Data tabs (limited to 1000 rows)"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '6px',
                ...
                >
                  ...
                  {downloadLoading ? 'Loading...' : 'Excel (1K)'}
                </button>
                <button
