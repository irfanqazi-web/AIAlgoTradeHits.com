"""
Stock 5-Minute Data Fetcher for Top 30 Most Volatile Stocks
Fetches 5-minute OHLC data for top 30 day-trading stocks with technical indicators
"""
...
TABLE_ID = '5min_stock'
HOURLY_TABLE = 'hourly_stock'


def get_top30_trading_stocks():
    """Get top 30 most actively traded stocks for 5-minute tracking"""

    # TOP 30 stocks for day trading - highest volatility and volume
    # These stocks have the most intraday movement and trading opportunities
    TOP_30_5MIN_STOCKS = [
        # Extreme Volatility - Meme & High-Beta (10)
        'TSLA',   # Tesla - king of volatility
        'GME',    # GameStop - meme king
        'AMC',    # AMC Entertainment
        'NVDA',   # NVIDIA - semiconductor volatility
        ...
        'PLTR',   # Palantir - AI volatility

        # High-Volume Day Trading Stocks (10)
        'SPY',    # S&P 500 ETF - highest volume
        'QQQ',    # Nasdaq 100 - tech volatility
        'TQQQ',   # 3x Nasdaq Bull
        'SQQQ',   # 3x Nasdaq Bear
        'AAPL',   # Apple - high volume
        'AMD',    # AMD - semiconductor swings
        'HOOD',   # Robinhood - retail trading
        'SOFI',   # SoFi - fintech
        'LCID',   # Lucid - EV volatility
        'RIVN',   # Rivian - EV startup

        # Momentum & Growth Volatility (10)
        'ARKK',   # ARK Innovation - Cathie Wood
        'PLUG',   # Plug Power - hydrogen volatility
        'NIO',    # Nio - Chinese EV
        'SNAP',   # Snap - social media swings
        'UBER',   # Uber - rideshare volatility
        'ABNB',   # Airbnb - travel volatility
        'ROKU',   # Roku - streaming
        'PTON',   # Peloton - fitness volatility
        'ZM',     # Zoom - remote work
        'RBLX',   # Roblox - gaming
    ]

    logger.info(f"Using curated list of top 30 stocks for 5-minute tracking")
    logger.info(f"Symbols: {TOP_30_5MIN_STOCKS}")

    return TOP_30_5MIN_STOCKS


def calculate_technical_indicators(df):
    """Calculate technical indicators for a dataframe (simplified for 5-min data)"""

    if len(df) < 50:
        logger.warning(f"Limited data for indicators: {len(df)} rows")
...
def fetch_stock_5min_data(symbol):
    """Fetch 5-minute data for a single stock with data cleaning"""
    try:
        ...
        if df.empty:
            logger.warning(f"No 5-min data returned for {symbol}")
            return None
        ...
        if df.empty:
            logger.warning(f"No valid 5-min data after cleaning for {symbol}")
            return None
        ...
        # Select only the latest 5-minute candle
        df = df.tail(1)
        ...
        logger.info(f"✓ Fetched {len(df)} 5-min records for {symbol}")
        return df

    except Exception as e:
        logger.error(f"✗ Error fetching 5-min data for {symbol}: {str(e)}")
        return None
...
def upload_to_bigquery(df):
    ...
    try:
        ...
        logger.info(f"✓ Uploaded {len(df)} new 5-min records to BigQuery")
        return len(df)
...
def main(request):
    ...
    logger.info("=" * 70)
    logger.info("STOCK 5-MINUTE DATA FETCHER (TOP 30 VOLATILE STOCKS) - STARTING")
    logger.info("=" * 70)

    start_time = time.time()

    # Get top 30 trading stocks (curated list)
    top_symbols = get_top30_trading_stocks()

    if not top_symbols:
        logger.warning("No symbols configured, exiting")
        return {'success': False, 'error': 'No symbols configured'}, 200
    ...
    for i, symbol in enumerate(top_symbols, 1):
        logger.info(f"\n[{i}/{len(top_symbols)}] Processing {symbol}...")

        df = fetch_stock_5min_data(symbol)
    ...
    logger.info("=" * 70)
    logger.info("STOCK 5-MINUTE DATA FETCHER - COMPLETE")
    logger.info("=" * 70)
    logger.info(f"✓ Success: {success_count}/{len(top_symbols)} stocks")
