"""
Kraken Pro 5-Minute Top-10 Gainers Data Fetcher with Technical Indicators
Fetches 5-minute OHLC data for top 10 hourly gainers and calculates technical indicators
"""
...
HOURLY_TABLE_ID = 'hourly_crypto'
FIVE_MIN_TABLE_ID = '5min_crypto'

def calculate_technical_indicators(df):
    """Calculate all technical indicators for a dataframe"""

    if len(df) < 50:
        logger.warning(f"Limited data for indicator calculation: {len(df)} rows")
...
def get_top30_trading_pairs():
    """Get top 30 most actively traded crypto pairs for 5-minute tracking"""

    # Top 30 cryptos by trading volume and volatility
    # These are the most popular for day trading
    TOP_30_5MIN_PAIRS = [
        # Top 10 - Major cryptos with highest volume
        'XXBTZUSD',      # Bitcoin
        'XETHZUSD',      # Ethereum
        'SOLUSD',        # Solana
        'XRPUSD',        # Ripple
        ...
        # Next 10 - High volatility altcoins
        'ATOMUSD',       # Cosmos
        'UNIUSD',        # Uniswap
        'LTCUSD',        # Litecoin
    ...
    logger.info(f"Using curated list of top 30 crypto pairs for 5-minute tracking")
    logger.info(f"Pairs: {TOP_30_5MIN_PAIRS}")

    return TOP_30_5MIN_PAIRS


def fetch_5min_data_for_pairs(pairs):
    """Fetch 5-minute OHLC data for specific pairs with technical indicators"""
    ...
    # Fetch enough data for indicator calculation (250 candles = ~20 hours)
    hours_ago = datetime.now() - timedelta(hours=20)
    ...
    for idx, pair in enumerate(pairs, 1):
        try:
            ...
            # Keep all recent records (last hour worth)
            one_hour_ago_ts = int((datetime.now() - timedelta(hours=1)).timestamp())
            df_recent = df_pair[df_pair['timestamp'] >= one_hour_ago_ts].copy()
            df_recent['fetched_at'] = datetime.now()

            all_processed_data.append(df_recent)
    ...
    logger.info(f"Processed data for {len(all_processed_data)} pairs")
...
def append_to_bigquery(df):
    """Append data to BigQuery 5-minute table with all technical indicators"""
...
def main(request):
    ...
    logger.info("="*60)
    logger.info("Starting 5-Minute Top-30 Crypto Pairs Data Fetch with Technical Indicators")
    logger.info(f"Timestamp: {datetime.now()}")
    logger.info("="*60)

    # Step 1: Get top 30 trading pairs (curated list)
    top_pairs = get_top30_trading_pairs()

    if not top_pairs:
        logger.warning("No pairs configured, skipping fetch")
        logger.info("="*60)
        return 'No pairs configured', 400

    # Step 2: Fetch 5-minute data with indicators for those pairs
    ...
    if df is not None and not df.empty:
        ...
        if success:
            logger.info("5-minute data fetch with indicators completed successfully!")
            logger.info("="*60)
            return '5-minute crypto data fetch completed successfully', 200
        else:
        ...
        logger.info("="*60)
        return 'No 5-minute data fetched', 500
