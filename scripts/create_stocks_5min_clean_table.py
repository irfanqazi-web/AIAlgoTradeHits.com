"""
Create stocks_5min_clean table with complete 85-field schema
5-minute data for top 10-60 symbols, 1 month history
Partitioned by DAY (1 month = ~30 days)
"""
...
# Define the 85-field schema (identical to daily and hourly tables)
schema = [
    ...
    bigquery.SchemaField("log_return", "FLOAT", mode="NULLABLE",
                        description="Logarithmic Return (1-period)"),
    bigquery.SchemaField("return_2w", "FLOAT", mode="NULLABLE",
                        description="Forward Return (short-term equivalent)"),
    bigquery.SchemaField("return_4w", "FLOAT", mode="NULLABLE",
                        description="Forward Return (medium-term equivalent)"),
...
table_id = "stocks_5min_clean"
full_table_id = f"aialgotradehits.{dataset_id}.{table_id}"

# Create table with partitioning and clustering
table = bigquery.Table(full_table_id, schema=schema)

# Partition by DAY (1 month = ~30 days, well within 4000 limit)
...
table.description = """
Clean consolidated 5-minute stock data table with 85 fields.
Includes OHLCV data, 40+ technical indicators, and ML features.
Source: v2_stocks_5min
Covers top 10-60 most liquid symbols with 1 month of 5-minute data.
Updated every 15-30 minutes during market hours (9 AM - 4 PM ET).
Prioritizes: AAPL, MSFT, NVDA, TSLA, AMZN, GOOGL, META, SPY, QQQ, AMD
"""
...
try:
    ...
    print(f"    - Partitioned by: DAY(datetime)")
    print(f"    - Clustered by: symbol, sector, exchange")
    print("="*80)
    print("\nTable created successfully!")
    print("\nNext steps:")
    print("1. Migrate 5-minute data from v2_stocks_5min")
    print("2. Calculate all technical indicators for top 10 stocks")
    print("3. Set up Cloud Function for 5-minute updates")
